...
class DownsampleNetworkResNet18V1(nn.Module):
    ...
    def __init__(self, in_channels=3):
        ...
        # Adapt the stem convolution to the input modality. When working with
        # grayscale images, the pre-trained RGB filters are fused (summed over
        # the channel dimension) so that the ImageNet features are preserved.
        if in_channels != 3:
...
class RetrieveROIModule(AbstractMILUnit):
    ...
    def forward(self, x_original, cam_size, h_small):
        ...
        for _ in range(self.num_crops_per_class):
            ...
            rpn_device = (torch.device("cuda:{}".format(self.gpu_number))
                          if self.parameters["device_type"] == "gpu" else None)
            mask = tools.generate_mask_uplft(current_images, crop_shape_adjusted, max_pos, rpn_device)
            current_images = current_images * mask
        return torch.cat(all_max_position, dim=1)
