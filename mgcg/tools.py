...
def generate_mask_uplft(input_image, window_shape, upper_left_points, device=None):
    """
    ...
    :param device: target torch device; defaults to the device of
                   ``upper_left_points`` (keeps CPU runs working).
    """
    N, C, H, W = input_image.size()
    ...
    if device is None:
        device = upper_left_points.device
